...
public static class BuilderUtil
{
	...
	public static void HideTileMapPrefabs(tk2dTileMap tileMap)
	{
		...
		{
			GameObject val = (tileMap.PrefabsRoot = tk2dUtil.CreateGameObject("Prefabs"));
			val.transform.parent = tileMap.renderData.transform;
		...
		for (int m = 0; m < tilePrefabsListCount; m++)
		{
			...
			if (!array[m])
			{
				...
				if (num >= 0 && num < array2.Length && array2[num] != (Object)null)
				{
					array[m] = true;
				}
			}
			...
			for (int j = y; j != y2; j += dy)
			{
				for (int k = x; k != x2; k += dx)
				{
					...
					else if (!flag && (Object)(object)chunk.gameObject == (Object)null)
					{
						...
						GameObject obj = (chunk.gameObject = tk2dUtil.CreateGameObject(name));
						obj.transform.parent = layer.gameObject.transform;
						MeshFilter val = tk2dUtil.AddComponent<MeshFilter>(obj);
						tk2dUtil.AddComponent<MeshRenderer>(obj);
